...
// FIX: itemUse hanya fire saat klik di udara kosong (tanpa blok di crosshair).
// Di server nyata player hampir selalu ngarahin cursor ke blok, jadi menu
// tidak pernah terbuka. Handler ini menangkap klik kanan ke blok non-chest.
world.beforeEvents.itemUseOn.subscribe(ev => {
  const player = ev.source;
  if (ev.itemStack?.typeId !== CFG.TRIGGER) return;
  if (ev.block?.typeId === "minecraft:chest") return; // ditangani chest handler di security.js
  if (activePlayers.has(player.id) || pendingChestInteract.has(player.id)) return;
  if ((lastPull.get(player.id) ?? 0) + CFG.PULL_CD > system.currentTick) {
    system.run(() => player.sendMessage("§e[Gacha] Tunggu sebentar!"));
    return;
  }
  ev.cancel = true; // cegah interaksi default ke blok
  system.run(async () => {
    activePlayers.set(player.id, "__hub__");
    try {
      const claimed = claimPend(player);
